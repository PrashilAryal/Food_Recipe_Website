@extends('layout.main')
@section('content')
    


<div class="bradcam_area  overlay" style="background-image : url('{{asset('img/recipe/img1.jpg')}}');">
    
</div>



                          

   

    

    
    <div class="recipe_area plus_padding mt-5">
        <div class="container">
            <div class="row">
            <div class="col-xl-4 col-lg-4 col-md-6">
            <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/sandn.jpg" alt="">
                        </div>
                        <h3>sandwich</h3>
                        
                        <p>Time Needs: 15min Mins</p>
                        <a href="{{route('ViewRecipe')}}" class="btn btn-outline-secondary"> View Full Recipe</a>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/pork.jpg" alt="">
                        </div>
                        <h3>Pork</h3>
                        
                        <p>Time Needs: 30 Mins</p>
                        <button type="button" class="btn btn-outline-secondary">View Full Recipe</button>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-lg-4">
                <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/rec3.jpg" alt="">
                        </div>
                        <h3>MOMO</h3>
                        
                        <p>Time Needs: 30 Mins</p>
                        <button type="button" class="btn btn-outline-secondary">View Full Recipe</button>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/rec1.jpg" alt="">
                        </div>
                        <h3>Chowmein</h3>
                        
                        <p>Time Needs: 30 Mins</p>
                        <button type="button" class="btn btn-outline-secondary">View Full Recipe</button>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/pizza1.jpg" alt="">
                        </div>
                        <h3>Pizza</h3>
                        
                        <p>Time Needs: 30 Mins</p>
                        <button type="button" class="btn btn-outline-secondary">View Full Recipe</button>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6">
                    <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/bur.jpg" alt="">
                        </div>
                        <h3>Burger</h3>
                        
                        <p>Time Needs: 30 Mins</p>
                        <button type="button" class="btn btn-outline-secondary">View Full Recipe</button>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6">
                    <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/egg.jpg" alt="">
                        </div>
                        <h3>Egg curry</h3>
                        
                        <p>Time Needs: 30 Mins</p>
                        <button type="button" class="btn btn-outline-secondary">View Full Recipe</button>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6">
                    <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/ice.jpg" alt="">
                        </div>
                        <h3>Vanilla ice-cream</h3>
                        
                        <p>Time Needs: 30 Mins</p>
                        <button type="button" class="btn btn-outline-secondary">View Full Recipe</button>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single_recipe text-center mb-20">
                        <div class="recipe_thumb">
                            <img src="img/recipe/thukpa.jpg" alt="">
                        </div>
                        <h3> Vegan noodles</h3>
                        
                        <p>Time Needs: 30 Mins</p>
                        <button type="button" class="btn btn-outline-secondary">View Full Recipe</button>
                    </div>
                </div>
               
            </div>
        </div>
    </div>
@endsection